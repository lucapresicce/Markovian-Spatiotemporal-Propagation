...
# ============================================================
#  FFBS-DYNBPS-v2.R
#  Optimised R wrapper for the spFFBS_v2 C++ backend.
# ============================================================
...
#' Convert a full prior covariance matrix to block form for spFFBS_v2
#'
#' @param m0   (p+n) × q prior mean matrix.
#' @param C0   (p+n) × (p+n) prior covariance matrix (full).
#' @param nu0  Prior degrees of freedom (scalar).
#' @param Psi0 q × q prior scale matrix.
#' @param p    Number of regression coefficients (integer).
#'
#' @return A list with elements m, C_bb, C_bw, C_ww, nu, Psi
#'   ready to pass as \code{prior} to \code{spFFBS_v2}.
#'
#' @export
make_prior_v2 <- function(m0, C0, nu0, Psi0, p) {
  n <- nrow(C0) - p
...
#'
#' @return n × J weight matrix.
#' @keywords internal
compute_Wt_v2 <- function(out_FF, tau, phi,
  ...
  if (verbose) {
    cat(sprintf("  Weight matrix: %d × %d  (%.2f s)\n\n", nrow(Wi), ncol(Wi), elapsed))
    cat("====================================================\n\n")
  }
...
#' spFFBS_v2: Spatiotemporal Bayesian Pipeline (optimised)
#'
#' @description
...
#' \itemize{
#'   \item \code{G} is now a \emph{p×p matrix} (\code{G_beta}) plus a scalar
...
#'   \item \code{X} (n×p×T array of covariates) replaces the \code{P} cube.
#'     The observation matrix \eqn{P_t = [X_t \mid I_n]} is formed implicitly
#'     inside C++.
#'   \item \code{prior} should be in block form; use \code{make_prior_v2()}
#'     to convert a full \eqn{C_0} matrix if needed.
#' }
#'
#' @param Y     Response array, n × q × T.
#' @param X     Covariate array, n × p × T. Replaces \code{P}.
#' @param D     n × n spatial distance matrix.
#' @param grid  List with elements \code{tau} and \code{phi} (numeric vectors).
#' @param prior Block-form prior list; see \code{\link{make_prior_v2}}.
#'   Elements: \code{m} ((p+n)×q), \code{C_bb} (p×p), \code{C_bw} (p×n),
#'   \code{C_ww} (n×n), \code{nu} (scalar), \code{Psi} (q×q).
#'   If the old-style full \code{C} is supplied instead of \code{C_bb/C_bw/C_ww},
#'   the wrapper auto-converts using \code{make_prior_v2()}.
#' @param G_beta  p×p upper-left block of the system matrix G.
#'   Default \code{NULL} → identity.
#' @param rho     Scalar multiplier for the lower-right n×n identity block of G.
#'   Default 1.
...
#' @param X_all    n×p×(T+tnew) covariate array for forecasting (required when
#'   \code{do_forecast = TRUE}).
#' @param spatial  List for spatial interpolation:
#'   \code{list(crd, crdtilde, Xtilde, t)}.
#'   \code{Xtilde} should be a u×p matrix (covariates at prediction sites).
#' @param n_threads  Number of OpenMP threads. Default 1.
#' @param verbose    Logical; print progress? Default TRUE.
#'
...
#'     \item{\code{Wi}}{n×J dynamic weight matrix.}
#'     \item{\code{Wglobal}}{J×1 global (averaged) weight vector.}
#'     \item{\code{BS}}{(if \code{do_BS}) T-list of n×q×L sample cubes.}
#'     \item{\code{forecast}}{(if \code{do_forecast}) List with \code{Y_pred}
#'       array of dimensions n×q×L×T.}
...
spFFBS_v2 <- function(
  ...
  if (verbose) {
    cat("\n====================================================\n")
    cat("      Welcome to spFFBS_v2 Bayesian Engine\n")
    cat("====================================================\n\n")
  }
  ...
  if (is.null(G_beta)) {
    ...
    if (!is.matrix(G_beta) || nrow(G_beta) != p_dim || ncol(G_beta) != p_dim)
      stop(sprintf("G_beta must be a %d × %d matrix.", p_dim, p_dim))
  }

  if (!is.null(prior$C) && is.null(prior$C_bb)) {
    if (verbose) cat("  Auto-converting full prior C to block form...\n")
    prior <- make_prior_v2(prior$m, prior$C, prior$nu, prior$Psi, p_dim)
  }
  ...
  Wi      <- compute_Wt_v2(out_FF, tau = grid$tau, phi = grid$phi,
  ...
  if (do_forecast) {
    ...
    if (is.null(X_all))
      stop("do_forecast = TRUE requires X_all: the n×p×(T+tnew) covariate array.")
    ...
    # Dynamic weight matrix: J × (tmax-1), each col = Wglobal
  ...
  if (do_spatial) {
    ...
    Xtilde   <- spatial$Xtilde   # u × p matrix OR u × p × length(t) array
    ...
    if (is.array(Xtilde) && length(dim(Xtilde)) == 3L) {
      if (dim(Xtilde)[1L] != u || dim(Xtilde)[2L] != p_dim)
        stop(sprintf(
          "spatial$Xtilde array must have dimensions u=%d × p=%d × K. Got %s.",
          u, p_dim, paste(dim(Xtilde), collapse = " × ")))
      K_sp <- dim(Xtilde)[3L]
      ...
      if (!is.matrix(Xtilde) || nrow(Xtilde) != u || ncol(Xtilde) != p_dim)
        stop(sprintf(
          paste0("spatial$Xtilde must be a u=%d × p=%d matrix, ",
                 "or a u × p × length(t) array."),
          u, p_dim))
    ...
    for (idx_t in seq_along(t_vec)) {
      ...
      # sp_arr is (2u*q) × L column-major; reshape to (2u, q, L)
  ...
  if (verbose) {
    cat("====================================================\n")
    cat("    spFFBS_v2 pipeline completed successfully!\n")
    cat("====================================================\n\n")
  }
